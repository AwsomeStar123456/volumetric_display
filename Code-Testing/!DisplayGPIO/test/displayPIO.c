...
#define NUM_ANIMATIONS 3 //Number of animations we have in the animation array.
int animations_currentframe = 0; //Which animation to start on.

uint16_t (*animations[NUM_ANIMATIONS])[24][9] = {water, intel, rotatingcube}; //Array of animations.
uint16_t animations_framecount[NUM_ANIMATIONS] = {NUM_WATER_FRAMES, NUM_INTEL_FRAMES, NUM_ROTATINGCUBE_FRAMES}; //Number of frames in each animation.
uint16_t animations_framebeforenext[NUM_ANIMATIONS] = {1, 3, 2}; //Number of frames to repeat before moving to the next frame.

uint16_t (*activeFrame)[24][9]; 
