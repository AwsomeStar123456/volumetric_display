...
#define NUM_ANIMATIONS 9 //Number of animations we have in the animation array.
int animations_currentframe = 0; //Which animation to start on.
const uint16_t (*animations[NUM_ANIMATIONS])[24][9] = {doughnut, bouncyball, intel, flappybird, sinewave, fire, water, rain, fireworks}; //Array of animations.
uint16_t animations_framecount[NUM_ANIMATIONS] = {NUM_DOUGHNUT_FRAMES, NUM_BOUNCYBALL_FRAMES, NUM_INTEL_FRAMES, NUM_FLAPPYBIRD_FRAMES, NUM_SINEWAVE_FRAMES, NUM_FIRE_FRAMES, NUM_WATER_FRAMES, NUM_RAIN_FRAMES, NUM_FIREWORKS_FRAMES}; //Number of frames in each animation.
uint16_t animations_framebeforenext[NUM_ANIMATIONS] = {1, 1, 3, 1, 1, 1, 1, 1, 1}; //Number of frames to repeat before moving to the next frame.
